...
const CHANNEL_NAME = 'WOO'
const FEE_RATE = 0.40 // WOO 입점 수수료 (고정)
...
// 스타일넘버만으로, 혹은 상품명으로 폴백 매칭될 수 있게 해주는 헬퍼. WOO는 판매가/결제금액을
// 직접 입력하므로 이 인덱스는 "원가(cost_price)" 매칭에만 사용하고, 판매가에는 쓰지 않는다.
...
export default function WooPage() {
  ...
  async function loadData() {
    ...
    const prevFeeArr = prevGrossArr.map(g => Math.round(g * FEE_RATE))
  ...
  // WOO는 판매가~결제금액을 전부 직접 입력한 값(이미 원화 환산 완료)을 그대로 사용한다.
  // 재고(items) 테이블은 원가(cost_price) 매칭 용도로만 쓰고, 판매가는 절대 items.sell_price로 덮어쓰지 않는다.
  async function handleUpload(file: File) {
    ...
    try {
      ...
      const idx = {
        ...
        amount: headers.findIndex(h => h.includes('판매가')),
      ...
      const { data: allItems } = await supabase.from('items').select('sku, name, option_name, cost_price')
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        // 판매가/결제금액은 파일에 입력된 값 그대로 사용 (수동 입력값, 원가공비 매칭과는 무관)
        ...
        // 원가(cost_price)만 스타일넘버 기준으로 재고와 매칭. 판매가는 items.sell_price를 쓰지 않는다.
        ...
        lineRecords.push({
          ...
          registered_sell_price: 0,
        })
      }

      let deleteErrorMsg = ''
      if (minDate && maxDate) {
        const { error: delErr } = await supabase.from('woo_settlement_lines')
      ...
      if (lineRecords.length > 0) {
        const { error } = await supabase.from('woo_settlement_lines').insert(lineRecords)
      ...
      setUploadMsg(
        ...
        (unmatchedCount > 0 ? `\n⚠ 재고 미매칭(원가) ${unmatchedCount}건 (예: ${unmatchedSamples.map(s => `"${s}"`).join(', ')}) — "원가 확인" 팝업에서 확인해보세요` : '') +
  ...
  async function runClaudeAI() {
    ...
    try {
      ...
      const prompt = `너는 패션 브랜드 "헤오진"의 데이터 분석가야. 아래는 "${CHANNEL_NAME}" 채널의 ${year}년 ${viewMonthIdx + 1}월 기준 데이터야.

[핵심 지표]
- 총 매출액: ${formatWon(totalGross)}
- 순매출액(수수료 차감 후): ${formatWon(totalNet)}
- 순수익(공헌이익): ${formatWon(totalProfit)}

[규칙 기반 인사이트]
${bulletLines || '(참고할 만한 규칙 기반 인사이트 없음)'}

위 내용을 바탕으로 실제 데이터 분석 관점에서 핵심 요약과 구체적인 다음 액션 제안을 한국어로 간결하게 작성해줘.`
  ...
  function renderRevenueLabel(props: any) {
    ...
    return (
      <text x={x + width / 2} y={y - 4} textAnchor="middle" fontSize={9} fontWeight={700} fill="#8b5cf6">
    ...
    <div>
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ width: 12, height: 12, borderRadius: '50%', background: '#8b5cf6', display: 'inline-block' }}></span>
          <h2 className="page-title" style={{ margin: 0 }}>WOO 손익 & 업로드</h2>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 12, color: '#8b5cf6', fontWeight: 600 }}>수수료 {(FEE_RATE * 100).toFixed(0)}% · 금액은 직접 입력한 원화 기준</span>
        </div>
      </div>
      ...
      {loading ? <div className="loading">로딩 중...</div> : (
        <>
          ...
          {(() => {
            const insights = generateInsights()
            const groups = [
              { title: '📈 매출 현황', items: insights.sales, color: '#8b5cf6' },
              { title: '👕 아이템 피드백', items: insights.items, color: '#f43f5e' },
            ]
            return (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <button onClick={() => fileRef.current?.click()} disabled={uploading}
                    style={{ border: '1px solid #ddd6fe', background: '#fff', color: '#7c3aed', cursor: 'pointer', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6 }}>
                    {uploading ? '업로드 중...' : '📤 업로드'}
                  </button>
                </div>
              </div>
            ...
            <div style={{ background: '#f5f3ff', border: '1px solid #94a3b8', borderRadius: 16, padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          ...
          {uploadMsg && (
            <div style={{ marginBottom: 20, padding: '10px 14px', background: '#f5f3ff', borderRadius: 10, fontSize: 13, color: '#7c3aed', fontWeight: 600, whiteSpace: 'pre-line' }}>
              ✓ {uploadMsg}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
            <div style={{ background: '#fff', border: '1px solid #94a3b8', borderRadius: 16, padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, gap: 8, flexWrap: 'wrap' }}>
                <div style={{ fontWeight: 700, fontSize: 15 }}>WOO 베스트 상품 순위</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <button onClick={() => setShowByOption(v => !v)}
                    style={{ border: '1px solid #94a3b8', background: showByOption ? '#f5f3ff' : '#fff', color: '#7c3aed', cursor: 'pointer', fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 6 }}>
                      ...
                      <tbody>
                        {displayRows.map((it, i) => (
                          <tr key={it.key} style={{ borderBottom: '1px solid #f1f5f9' }}>
                            <td style={{ padding: '8px 10px', textAlign: 'center', fontWeight: 700, color: i < 3 ? '#7c3aed' : '#94a3b8' }}>{i + 1}</td>
                            ...
                            <td style={{ padding: '8px 10px', textAlign: 'center', fontWeight: 700, color: '#7c3aed' }}>{it.qty}건</td>
                ...
                <>
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={categorySales.map(c => ({ name: c.category, 판매량: c.qty }))} layout="vertical" margin={{ left: 10 }}>
                      ...
                      <Tooltip formatter={(v: number) => `${v}건`} />
                      <Bar dataKey="판매량" fill="#f43f5e" radius={[0, 4, 4, 0]}>
                        <LabelList dataKey="판매량" position="right" formatter={(v: number) => v ? `${v}건` : ''} style={{ fontSize: 11, fill: '#f43f5e', fontWeight: 700 }} />
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 16 }}>
                    {categorySales.map((c, i) => (
                      <div key={c.category} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', background: '#f8fafc', borderRadius: 10 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: i < 3 ? '#f43f5e' : '#94a3b8', width: 20 }}>{i + 1}</span>
                        <div style={{ flex: 1, fontSize: 13, fontWeight: 600 }}>{c.category}</div>
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#f43f5e' }}>{c.qty}건</span>
                      </div>
                    ))}
                  </div>
                </>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, whiteSpace: 'nowrap' }}>
                  ...
                  <tbody>
                    {orderRows.map((r: any) => {
                      ...
                      const profit = settlement - (r.matched_cost || 0) - shippingRateFor(r.settle_date || '')
